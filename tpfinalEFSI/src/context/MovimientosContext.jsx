...
export function MovimientosProvider({ children }) {
  ...
  useEffect(() => {
    if (!movimientos || movimientos.length === 0) setMovimientos(movimientosMock)
  ...
  const removeMovimiento = (id) => setMovimientos(prev => prev.filter(m => String(m.id) !== String(id)))
  const resetDatos = () => setMovimientos(movimientosMock)
  const clearDatos = () => setMovimientos([]) // deja vacío
